#!/usr/bin/env python3
"""
PHASE 5 - Self-Correcting Speech Translation (Method 2: MBR Decoding)
Architecture: Mic → ASR → MT(beam=1) → QE gate → MT(beam=5 MBR) → TTS
"""
...
def mbr_select(candidates):
    """Pick candidate with highest avg ChrF against all peers."""
    if len(candidates) == 1:
...
try:
    while True:
        ...
        if qe_score < QE_THRESHOLD:
            print(f"  ⚡ QE below threshold — generating {N_BEAMS} candidates for MBR...")
            ...
            if ok2 and beams:
                print(f"  Beams [{beam_lat:.0f}ms]: {len(beams)} candidates")
                for i, b in enumerate(beams):
                    print(f"    [{i+1}] \"{b[:60]}\"")

                # ── MBR selection ─────────────────────────────
                t0 = time.time()
                final_translation, mbr_score = mbr_select(beams)
                mbr_lat = (time.time() - t0) * 1000
                correction_applied = True
                print(f"  MBR  [{mbr_lat:.0f}ms]: consensus score={mbr_score:.4f}")
                print(f"  ✓ Selected: \"{final_translation}\"")
        else:
