...
try:
    ...
    print("[WARNING] noisereduce not installed - noise reduction disabled")
...
class MicrophoneAudioCapture:
    ...
    def __init__(self, 
        ...
        self.noise_reduction_strength = 0.4  # 40% reduction in noise to reduec background stuff
   
    def calibrate_noise_profile(self, duration=2.0):
        if not NOISEREDUCE_AVAILABLE:
            return False, "noisereduce library not installed"
        
        print(f"\n{'='*60}")
        print(f"NOISE CALIBRATION")
        print(f"{'='*60}")
        print(f"Recording {duration}s of background noise...")
        print("Please stay SILENT - let only the fan noise run this is for initialisation")
        print(f"{'='*60}\n")
        
        try:
            # Record noise sample
            noise_samples = int(self.sample_rate * duration)
            noise = sd.rec(noise_samples,
    ...
    def _apply_noise_reduction(self, audio_chunk):
        ...
        try:
            ...
            return audio_chunk  # Return original if denoising fails
    ...
    def start_recording(self):
        ...
        try:
            ...
            nr_status = "ENABLED" if self.noise_reduction_enabled else "DISABLED"
            return True, f"Recording started (sample_rate={self.sample_rate}Hz, noise_reduction={nr_status})"
            
        except Exception as e:
            return False, f"Failed to start recording: {str(e)[:100]}"
    
    def record_until_silence(self, timeout_sec=10):
        """
        ...
        while time.time() - start_time < timeout_sec:
            if len(self.audio_buffer) > 0:
                ...
                if energy > self.silence_threshold:
                    ...
                    silent_chunks = 0
                    self.recording_buffer.append(chunk_denoised.copy())
